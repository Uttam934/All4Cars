...
public class PinCodeRoundView extends RelativeLayout {
    ...
    private void initializeView(AttributeSet attrs, int defStyleAttr) {
        if (attrs != null && !isInEditMode()) {
            final TypedArray attributes = mContext.getTheme().obtainStyledAttributes(attrs, com.github.orangegangsters.lollipin.lib.R.styleable.PinCodeView,
                    defStyleAttr, 0);

            mEmptyDotDrawableId = attributes.getDrawable(com.github.orangegangsters.lollipin.lib.R.styleable.PinCodeView_lp_empty_pin_dot);
            if (mEmptyDotDrawableId == null) {
                mEmptyDotDrawableId = getResources().getDrawable(com.github.orangegangsters.lollipin.lib.R.drawable.pin_code_round_empty);
            }
            mFullDotDrawableId = attributes.getDrawable(com.github.orangegangsters.lollipin.lib.R.styleable.PinCodeView_lp_full_pin_dot);
            if (mFullDotDrawableId == null) {
                mFullDotDrawableId = getResources().getDrawable(com.github.orangegangsters.lollipin.lib.R.drawable.pin_code_round_full);
            }

            attributes.recycle();

            LayoutInflater inflater = (LayoutInflater) mContext.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
            PinCodeRoundView view = (PinCodeRoundView) inflater.inflate(com.github.orangegangsters.lollipin.lib.R.layout.view_round_pin_code, this);
            mRoundContainer = (ViewGroup) view.findViewById( com.github.orangegangsters.lollipin.lib.R.id.round_container );
    ...
    public void setPinLength(int pinLength) {
        ...
        for (int i = 0; i < pinLength; i++) {
            ...
            if (i < mRoundViews.size()) {
                ...
                roundView = (ImageView) inflater.inflate(com.github.orangegangsters.lollipin.lib.R.layout.view_round, mRoundContainer, false);
            }
